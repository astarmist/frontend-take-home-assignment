...
describe("Test Search Result", () => {
  it("Search Header and Content renders correctly with the search term provided", () => {
    render(<SearchResult results={mockedData} searchText={searchText} />);
    const resultHeader = screen.getByText(/Showing/i);
    const resultContent = screen.getAllByText(/Child/i);

    expect(resultHeader).toBeInTheDocument();
    expect(resultContent[0]).toBeInTheDocument();
  });
});
